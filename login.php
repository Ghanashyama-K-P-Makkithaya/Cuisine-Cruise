...
if($_SERVER['REQUEST_METHOD'] == "POST"){
    $userdetail = $_POST['mailid'];
    $password = $_POST['password'];

    if(!empty($userdetail) && !empty($password)){
        $query = "SELECT * FROM `user` WHERE `MailID`='$userdetail' LIMIT 1";
        ...
        if($res && mysqli_num_rows($res)>0){
            ...
            if($user_data['Password'] === md5($password)){
                ...
                elseif ($user_data['UserType'] === 'Delivery Agent'){
                    header("Location: ./deliveryagent/index.php");
                }
...
<div class="cards">
        <div class="container-fluid">
            <h2 class="row justify-content-center display-1">Login</h2>
            <form action="" class="needs-validation" method="post">
                ...
                <div class="d-grid col-4 input-group mx-auto mb-3">
                    <button id="login" class="btn btn-warning text-center" type="submit">Login</button>
                </div>
